...
struct ExtractinvPass : public Pass {
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) YS_OVERRIDE
	{
		...
		// Cell type, port name -> parameter name.
		dict<pair<IdString, IdString>, IdString> invertible_pins;

		for (auto module : design->modules())
			if (module->get_blackbox_attribute())
				for (auto port : module->ports) {
					auto wire = module->wire(port);
					auto it = wire->attributes.find("\\invertible_pin");
					if (it != wire->attributes.end()) {
						IdString pname = RTLIL::escape_id(it->second.decode_string());
						invertible_pins[make_pair(module->name, wire->name)] = pname;
					}
				}
		...
		for (auto module : design->selected_modules())
		{
			...
			for (auto port : cell->connections()) {
				...
				auto it = invertible_pins.find(make_pair(cell->type, port.first));
				// Not invertible -- skip.
				if (it == invertible_pins.end())
					continue;
				auto it2 = cell->parameters.find(it->second);
				// Inversion not used -- skip.
				if (it2 == cell->parameters.end())
					continue;
				if (it2->second.size() != sig.size())
					log_error("The inversion parameter needs to be the same width as the port (%s.%s port %s parameter %s)", log_id(module->name), log_id(cell->type), log_id(port.first), log_id(it->second));
				RTLIL::Const invmask = it2->second;
				cell->parameters.erase(it->second);
